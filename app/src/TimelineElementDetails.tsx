...
export const TimelineElementDetails: React.FC<{artist: Artist, show: boolean}> = (props) => {
  ...
  return (<div className={className.join(' ')}>
        {props.artist.works.map(work => {
            return (
            <div key={work.name} className="vertical-timeline-element-details-element">
                <span className="vertical-timeline-element-details-element-desc">
                  <div>{work.name} - {work.date}</div>
                  <div className="vertical-timeline-element-details-element-desc-desc">{work.description}</div>
                </span>
            { work.img ? <img alt={work.name} className="vertical-timeline-element-details-element-img" src={work.img}></img>
