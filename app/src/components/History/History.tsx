...
export default function History(props: Props) {
  ...
  var events = Catalog.history
    .map((x) => x.events.map((y) => ({ period: x.name, ...y })))
    .flat();
  ...
  return (
    <div className="container">
      <header className="app-header">Histoire</header>
      <div>
        {details != null ? <div className="details">{details.text}</div> : null}
          ...
          {history.map((x, i) => {
            return (
              <div
                ...
                className="period"
                style={{
                  top: zoom * (x.start - startFirstPeriod) + "px",
                  height: zoom * (x.end - x.start) + "px",
                  marginLeft: i * 5 + "px",
                  color: periodColors[x.name],
                  borderColor: periodColors[x.name],
                }}
              >
          ...
          {events.map((x) => {
            return (
              <div
                ...
                className="event"
                style={{
                  top: zoom * (x.date - startFirstPeriod) + "px",
                  color: periodColors[x.period],
                }}
                onClick={() => {
                  setDetailsFromEvent(x);
                }}
              >
