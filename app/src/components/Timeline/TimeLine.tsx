...
export const TimeLine: React.FC<{ artists: Artist[] }> = ({ artists }) => {
  ...
  function clickArtist(
    artistName: string,
    refToFocus: React.RefObject<HTMLDivElement>
  ) {
    ...
    <div>
      <VerticalTimelineAny>
        {artists.map((x, i) => {
          ...
          const areWeAboveTheCollapsedArtistIfAny =
            showDetails &&
            i > artists.findIndex((x) => x.artist === showDetails);
          // This complicated crap is done to scroll at the right place depending of the "collapse" animation
          const scrollOffset =
            -20 - (areWeAboveTheCollapsedArtistIfAny ? timelineDetailSize : 0);

          return (
            <div key={x.artist} style={{ position: "relative" }}>
              <div
                ref={refToFocus}
                style={{
                  position: "absolute",
              ...
              <VerticalTimelineElementAny
                ...
                date={
                  x.lifePeriod.start +
                  " - " +
                  (x.lifePeriod.end ? x.lifePeriod.end : "today")
                }
